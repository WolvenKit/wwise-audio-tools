...
void bnk_t::bkhd_data_t::_read() {
    ...
    m_id = m__io->read_u4le();
    m_blank1 = m__io->read_bytes(4);
    if (!(blank1() == std::string("\x00\x00\x00\x00", 4))) {
        throw kaitai::validation_not_equal_error<std::string>(std::string("\x00\x00\x00\x00", 4), blank1(), _io(), std::string("/types/bkhd_data/seq/2"));
    }
    m_blank2 = m__io->read_bytes(4);
    if (!(blank2() == std::string("\x00\x00\x00\x00", 4))) {
        throw kaitai::validation_not_equal_error<std::string>(std::string("\x00\x00\x00\x00", 4), blank2(), _io(), std::string("/types/bkhd_data/seq/3"));
    }
    m_random = m__io->read_bytes((length() - 16));
}
...
void bnk_t::event_t::_read() {
    m_event_action_count = m__io->read_u4le();
    int l_event_actions = event_action_count();
    m_event_actions = new std::vector<uint32_t>();
    m_event_actions->reserve(l_event_actions);
...
void bnk_t::ss_state_group_t::_read() {
    ...
    int l_state_obj = settings_vary_from_default();
    m_state_obj = new std::vector<ss_state_obj_t*>();
    m_state_obj->reserve(l_state_obj);
...
void bnk_t::sound_structure_t::_read() {
    ...
    if (effects_count() > 0) {
        n_effect = false;
        int l_effect = effects_count();
        m_effect = new std::vector<ss_effect_t*>();
        m_effect->reserve(l_effect);
    ...
    int l_parameter_type = additional_parameter_count();
    m_parameter_type = new std::vector<int8_t>();
    m_parameter_type->reserve(l_parameter_type);
    for (int i = 0; i < l_parameter_type; i++) {
        m_parameter_type->push_back(m__io->read_s1());
    }
    int l_parameter_value = additional_parameter_count();
    ...
    if (user_aux_sends_exist()) {
        ...
        m_aux_busses->reserve(l_aux_busses);
    ...
    int l_state_group = state_group_count();
    m_state_group = new std::vector<ss_state_group_t*>();
    m_state_group->reserve(l_state_group);
...
void bnk_t::ss_rtpc_t::_read() {
    ...
    int l_point = num_points();
    m_point = new std::vector<ss_rtpc_point_t*>();
    m_point->reserve(l_point);
...
void bnk_t::music_segment_t::_read() {
    ...
    int l_child_obj = child_obj_count();
    m_child_obj = new std::vector<uint32_t>();
    m_child_obj->reserve(l_child_obj);
